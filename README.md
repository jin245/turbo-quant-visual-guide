...
2つの単位ベクトルの内積 $\langle x, y \rangle = 0.597$ を、5本のランダム射影で推定する。

**Step 1**: ランダム射影行列 $S$ を生成（各行が $s_k \sim N(0, I_4)$）
...
**Step 3**: x の射影を **1ビットに量子化**（符号だけ残す）。y はそのまま。

| | $\mathrm{sign}(s_k^\top x)$ | $s_k^\top y$（生の値） | 積 |
|--|:-:|--:|--:|
| $k=1$ | $-1$ | $-0.161$ | $+0.161$ |
| $k=2$ | $+1$ | $+0.730$ | $+0.730$ |
| $k=3$ | $-1$ | $-0.122$ | $+0.122$ |
...
$$\hat{v} = \frac{\sqrt{\pi/2}}{5} \times (0.161 + 0.730 + 0.122 + 0.904 + 0.314) = \frac{1.253}{5} \times 2.231 = 0.559$$

真の値 $0.597$ に対して誤差 $0.037$。たった5本の射影で、**x 側は符号（$\pm 1$）しか保存していない**のに内積を推定できている。射影数 $m$ を増やせば精度は任意に改善できる。
